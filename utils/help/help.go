...
func Usage() {
	...
	fmt.Fprintf(os.Stdout, "  -conflict MODE   -send/-recv: what to do about same-name files and interrupted transfers.\n")
	fmt.Fprintf(os.Stdout, "                   ask (default on a terminal): compares contents (SHA-256), then asks. Name already\n")
	fmt.Fprintf(os.Stdout, "                   holds an identical file -> rename/overwrite/skip; different -> rename/skip (\"rename\"\n")
	fmt.Fprintf(os.Stdout, "                   saves the NEW file under a new name, the old one is untouched). A leftover .part of an\n")
	fmt.Fprintf(os.Stdout, "                   interrupted transfer that matches the file -> continue/restart/skip. rename (default\n")
	fmt.Fprintf(os.Stdout, "                   when stdin is not a terminal): new name, no probing. Also: overwrite, skip, resume\n")
	fmt.Fprintf(os.Stdout, "                   (continue a .part; skip if the name already holds an identical file)\n")
