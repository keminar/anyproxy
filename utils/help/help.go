...
func Usage() {
	...
	fmt.Fprintf(os.Stdout, "                   (with -to <email> -via direct|relay; both are end-to-end encrypted and require the\n")
	fmt.Fprintf(os.Stdout, "                   receiver's websocket.client.receive.allow to list this machine's uuid; direct punches\n")
	fmt.Fprintf(os.Stdout, "                   through NAT, relay goes through the server B (no punching needed); default direct)\n")
	...
	fmt.Fprintf(os.Stdout, "                   (with -to <local dir>, default the current directory; -via direct|relay, default\n")
	fmt.Fprintf(os.Stdout, "                   direct; relay also needs the server B upgraded to this version)\n")
