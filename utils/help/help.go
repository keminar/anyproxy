...
func Usage() {
	...
	fmt.Fprintf(os.Stdout, "  -wol MAC[,MAC2,...] Send Wake-on-LAN magic packets and exit\n")
	fmt.Fprintf(os.Stdout, "                   (reuses -to/-via: -to ADDR[:PORT] is the broadcast target, default\n")
	fmt.Fprintf(os.Stdout, "                   255.255.255.255:9 limited broadcast; sending succeeded does not guarantee the target\n")
	fmt.Fprintf(os.Stdout, "                   woke up. Without -via, broadcasts from this machine; with -via <email>, asks that\n")
	fmt.Fprintf(os.Stdout, "                   subscriber -- who must be on the target machine's own LAN, since the target itself\n")
	fmt.Fprintf(os.Stdout, "                   is powered off and cannot run this command -- to broadcast instead. That subscriber\n")
	fmt.Fprintf(os.Stdout, "                   must list this email under its own websocket.client.receive.allow AND set wol: true\n")
	fmt.Fprintf(os.Stdout, "                   on that entry (default false; being allowed to send/receive files there does not by\n")
	fmt.Fprintf(os.Stdout, "                   itself permit this). Always relayed through the server B, no NAT punching, so -via\n")
	fmt.Fprintf(os.Stdout, "                   must be an email here, not \"direct\"/\"relay\")\n")
	...
	fmt.Fprintf(os.Stdout, "  -conflict MODE   -send/-recv: what to do about a same-name file or a leftover .part from an\n")
	fmt.Fprintf(os.Stdout, "                   interrupted transfer. MODE is one of:\n")
	fmt.Fprintf(os.Stdout, "                     ask       ask interactively, see below (default on a terminal)\n")
	fmt.Fprintf(os.Stdout, "                     rename    save the incoming file under a new name, no probing (default\n")
	fmt.Fprintf(os.Stdout, "                               when stdin is not a terminal); the existing file is untouched\n")
	fmt.Fprintf(os.Stdout, "                     overwrite replace the existing same-name file (also resumes a matching .part if found)\n")
	fmt.Fprintf(os.Stdout, "                     skip      leave the existing file/.part alone, transfer nothing\n")
	fmt.Fprintf(os.Stdout, "                     resume    continue a matching .part automatically; if the name already holds\n")
	fmt.Fprintf(os.Stdout, "                               an identical file, skip; if it differs, save under a new name\n")
	fmt.Fprintf(os.Stdout, "                   MODE=ask additionally prompts at the terminal, comparing contents (SHA-256) first --\n")
	fmt.Fprintf(os.Stdout, "                   these single letters are answers to that prompt, not values for -conflict itself:\n")
	fmt.Fprintf(os.Stdout, "                     name already exists      -> [r]ename / [o]verwrite (offered whether or not contents match) /\n")
	fmt.Fprintf(os.Stdout, "                                                 [s]kip (default)\n")
	fmt.Fprintf(os.Stdout, "                     a .part matches the file -> [c]ontinue / [r]estart from scratch /\n")
	fmt.Fprintf(os.Stdout, "                                                 [s]kip (default)\n")
	...
	fmt.Fprintf(os.Stdout, "For the rest of the recommended Linux kernel/network tuning (sysctl), run \"anyproxy -check\" to see\n")
	fmt.Fprintf(os.Stdout, "what's below recommendation, or \"anyproxy -check-fix\" (needs root) to apply it automatically.\n\n")
