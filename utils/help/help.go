...
func Usage() {
	...
	fmt.Fprintf(os.Stdout, "                   is powered off and cannot run this command -- to broadcast instead, subject to its\n")
	fmt.Fprintf(os.Stdout, "                   websocket.client.receive.allow. Always relayed through the server B, no NAT punching,\n")
	fmt.Fprintf(os.Stdout, "                   so -via must be an email here, not \"direct\"/\"relay\")\n")
