...
anyproxy 是一个通用的流量转发与内网穿透工具，设计目标是解决合法场景下的连通性问题——按域名转发流量、远程访问自己内网的设备、跨内网/跨机房打通服务、临时文件互传等。**它不是为规避网络监管、访问受限内容而设计的工具，也不擅长用于此类用途**，对此类用途下产生的后果由使用者自行承担，与本项目及作者无关。
