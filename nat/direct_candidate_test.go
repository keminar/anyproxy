...
func TestSelectPublicFamiliesCompeteOnRTT(t *testing.T) {
	v4 := directCandidate{Addr: "192.0.2.10:100", Source: candSrcReflectV4}
	v6 := directCandidate{Addr: "[2001:db8::53]:100", Source: candSrcReflectV6}
...
func TestSelectBiasBeatsSmallRTTGap(t *testing.T) {
	public := directCandidate{Addr: "192.0.2.10:100", Source: candSrcReflectV4}
...
func TestSelectBiasOrdering(t *testing.T) {
	...
	all := []candidateResult{
		{Cand: directCandidate{Addr: "192.0.2.10:1"}, RTT: rtt},
	...
	want := []string{"127.0.0.1:1", "169.254.0.1:1", "192.168.0.1:1", "192.0.2.10:1"}
	for _, w := range want {
...
func TestSelectSkipsFailures(t *testing.T) {
	got, err := selectCandidate([]candidateResult{
		...
		{Cand: directCandidate{Addr: "192.0.2.10:1"}, RTT: 90 * time.Millisecond},
	})
	if err != nil {
		t.Fatalf("select: %v", err)
	}
	if got.Addr != "192.0.2.10:1" {
...
func TestSelectAllFailedExplains(t *testing.T) {
	_, err := selectCandidate([]candidateResult{
		{Cand: directCandidate{Addr: "192.0.2.10:1", Source: candSrcReflectV4}, Err: errors.New("timeout")},
	...
	for _, want := range []string{"192.0.2.10:1", "timeout", "2400::1", "no route to host"} {
...
func TestDedupCandidates(t *testing.T) {
	got := dedupCandidates([]directCandidate{
		{Addr: "192.0.2.10:1", Source: candSrcReflectV4},
		{Addr: "192.0.2.10:1", Source: candSrcLocal}, // 同一端点从两个来源冒出来
