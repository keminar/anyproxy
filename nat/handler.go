...
func ConnectServer(cfg conf.WsClient, liveIndex int) {
	// connect 对单个 client: 块已经由 conf.Websocket.ClientList() 挡掉了(为空整块
	// 被排除, 不会走到这里), 但 clients[] 数组是整体一起返回、不逐项过滤的(见
	// ClientList 的注释)——这里必须再单独判一次, 否则数组里漏配 connect 的那一项
	// 会真的拿一个空地址反复拨号重试, 而不是"这条不生效"。
	if cfg.Connect == "" || cfg.User == "" || cfg.Email == "" {
		log.Printf("ws client entry incomplete (connect=%q user=%q email=%q), donot connect\n", cfg.Connect, cfg.User, cfg.Email)
		return
	}
	// 这条配置没有任何理由让常驻进程为它发起连接(见 conf.WsClient.
	// WantsPersistentConnect): 要么开着 sendRecvOnly 强制跳过, 要么
	// subscribe/forward/direct/directAccept/receive.dir 全是空的——服务端同样会因为
	// 它"什么都不做"而不断拒绝并断开(见 nat/conn.go emptySubscribeAllowed), 常驻
	// 进程连上去纯属陪跑, 还会在服务端刷一片"ignore, subscribe is empty"日志, 不如
	// 不发起。这条配置本身仍然完好, 可以被 pickClientConfig 挑去跑 -send/-recv,
	// 那条路径不经过这里, 不受影响。
	if !cfg.WantsPersistentConnect() {
		log.Printf("ws client entry email=%q has nothing for a persistent connection to do (no subscribe/forward/direct/directAccept/receive.dir, or sendRecvOnly is set); skip connect\n", cfg.Email)
		return
	}
...
//
// 密码强度不在这里卡: 弱密码校验只在服务端(validateServerUsers)做, 订阅端不判断,
// 这样新版本订阅端也能照常连尚未升级的旧服务端(或密码还没改达标的账号), 不会因为
// 本地这关先拒了、白白拨不出去——服务端那关本来就够了。
func (w *wsClientConn) connect(interrupt chan os.Signal) {
	...
	err = ch.auth(live.User, live.Pass, live.Key, live.Email, w.direct != nil, w.cfg.Receive.Dir != "")
	if err != nil {
		w.logf("auth: %v", err)
...
func httpRejectReason(resp *http.Response) string {
	...
	if resp.StatusCode == http.StatusForbidden {
		hint = "; this end's address is not in the server's websocket.server.allowIP (note IPv6 addresses rotate, prefer whitelisting a prefix/subnet)"
	}
...
// md5(user|pass|xtime)。direct 表示本端参与 QUIC 直连, receive 表示本端配了
// receive.dir, 两者都供服务端放行空订阅(见 AuthMessage.Direct / AuthMessage.Receive)。
func (h *ClientHandler) auth(user, pass, key, email string, direct, receive bool) error {
	if key != "" {
		return h.authKey(user, key, email, direct, receive)
	}
	xtime := time.Now().Unix()
	...
	msg := AuthMessage{User: user, Token: token, Xtime: xtime, Email: email, Direct: direct, Receive: receive}
	return h.ask(&msg)
}
...
// 都得能识别, 否则本端只会看到"连不上"。
func (h *ClientHandler) authKey(user, key, email string, direct, receive bool) error {
	...
	if err := h.c.WriteJSON(&AuthMessage{User: user, Email: email, KeyAuth: true, Direct: direct, Receive: receive}); err != nil {
		return err
	}
