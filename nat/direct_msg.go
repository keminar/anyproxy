...
// 一次直连的完整时序:
//
//	C --d_announce--> B          C 起 QUIC 监听后通告自己的端点+证书指纹
//	A --d_request-->  B          A 有入口连接进来, 报上自己的 UDP 端点和本次 token
//	B --d_punch-->    C          B 转交给 C: 有人要连你, 朝它的端点先打几个包
//	B --d_offer-->    A          B 回给 A: C 的端点+指纹(或错误原因)
...
const (
	METHOD_DIRECT_ANNOUNCE = "d_announce" //C -> B
...
func isDirectMethod(method string) bool {
	switch method {
	case METHOD_DIRECT_ANNOUNCE, METHOD_DIRECT_REQUEST, METHOD_DIRECT_PUNCH, METHOD_DIRECT_OFFER:
		return true
	}
	return false
}

// DirectAnnounce C 通告自己的 QUIC 端点。
//
// Endpoint 是**订阅方用 QUIC 那个 socket 问 UDP 反射器要来的**完整地址+端口, 不是本机
// 自报, 也不是服务端从 websocket 连接上观测的 —— websocket 是 TCP、是另一个 socket,
// 而一台机器可能同时持有多个全局 IPv6 地址(含会轮换的隐私临时地址), 内核按 RFC 6724
// 按目的地分别选源, 两者不保证相同。详见 nat/direct_reflect.go。
