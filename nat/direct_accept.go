...
func (d *directPeer) onPunch(msg *Message) {
	...
	//
	// p.Relay 时 peerCands 是 VPS 的中继端点 E, 不是 A 本人——本侧是居民、必须先打, 所以立即
	// 朝 E 打洞(不 park), 并在**第一个包确实发出去之后**经 B 给 VPS 捎一个 nudge
	// (punchOnlyThen 的回调): VPS 那台机器前面也有一层有状态防火墙/安全组, 它自己得主动朝
	// 本侧发一次包, 本侧后续的包才进得去它。顺序上是硬保证的——回调打在 UDP WriteTo 成功
	// 返回之后, nudge 再经 websocket 出去(见 direct_reflect.go 的 punchOnlyThen 与
	// docs/direct-relay-design.md)。
	//
	// p.PunchFirst: 发起方 A 声明它在受限 CGNAT 后、必须先发第一个包(它配了 directPunchFirst,
	// 仅用于直连场景)。此时本侧**先不打**, 把打洞停下(parkPunch), 等 A 开打后经 B 转来的
	// d_punching nudge 再打——这样 A 必先发出第一个包, 否则本侧的包先到 A 的 CGNAT, A 的
	// 映射会被毒化、双向全灭(见 docs/direct-punch-order.md)。nudge 丢了则
	// directPunchFirstDelay 到点兜底。不管哪种, d_ready 都照常立即回(下面 reply), 不拖慢
	// A 拿 offer。
	switch {
	case p.Relay:
		// 先朝 E 打出第一个包, 包确实发出去了才发 nudge(回调保证顺序, 不是靠调度赌)。
		d.punchOnlyThen(p.Token, peerCands, func() {
			if err := d.sendRelayNudge(p.Token); err != nil {
				d.logf("relay: send nudge for token %s failed: %v", shortToken(p.Token), err)
			}
		})
	case p.PunchFirst:
...
//     发出方 email), 触发 VPS 朝那条腿主动发包(fireRelayLeg)——这一步是中继能通的关键,
//     见 direct_relay.go 的 punchLeg。
...
// sendRelayNudge 居民(A 或 C)中继腿朝 E 打出第一个包后发的 nudge: 经 B 转给 VPS(B 认出这是
// 中继会话, 保留本机 email 当"腿"标签), 让 VPS 知道本侧已经打过、可以朝本侧打回来了 —— VPS
// 不主动发这一下, 它自己的防火墙就不会放行本侧的包。Email 填本机自己的(B 会用它认证过的
// c.Email 覆盖/核对, 不可伪造成别的腿)。
//
// 连发 3 次(0 / +600ms / +1.8s, 见 directRelayNudgeGaps): VPS 没有兜底定时器, nudge 丢了这条
// 腿就再也没有第二次机会; 重复到达无害(VPS 按腿 once 幂等)。返回值只是第一次发送的结果,
// 补发的结果只记日志——调用方用不上, 也不该因为补发失败就把整次连接判死。
func (d *directPeer) sendRelayNudge(token string) error {
	nudge := func() error {
		return d.send(METHOD_DIRECT_PUNCHING, 0, DirectPunching{Email: d.cfg.Email, Token: token})
	}
	err := nudge()
	go func() {
		for _, gap := range directRelayNudgeGaps {
			time.Sleep(gap)
			if e := nudge(); e != nil {
				d.logf("relay: resend nudge for token %s failed: %v", shortToken(token), e)
				return
			}
			d.logf("relay: resent nudge for token %s", shortToken(token))
		}
	}()
	return err
}
...
func (dc *directConn) serveStream(stream *quic.Stream) {
	...
	if head.Kind == directStreamAuth {
		...
		if _, err := stream.Write([]byte{directAuthACK, directCapReadyACK}); err != nil {
	...
	if head.Ready {
		if _, err := stream.Write([]byte{directAuthACK}); err != nil {
			d.logf("data ready ack to %s failed: %v", remote, err)
			return
		}
	}
