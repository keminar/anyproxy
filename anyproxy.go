...
func init() {
	...
	flag.StringVar(&gConflict, "conflict", "", "-send/-recv: what to do about same-name files and interrupted transfers: ask (the default on a terminal; compares contents, then asks: if the name already holds an identical file -> rename/overwrite/skip, different -> rename/skip; if an interrupted transfer's .part matches the start of the file -> continue/restart/skip), rename (save under a new name; the default when stdin is not a terminal), overwrite, skip, or resume (continue an interrupted .part; skip if the name already holds an identical file)")
