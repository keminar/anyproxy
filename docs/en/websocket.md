...
| `direct.relayPublic` | optional, explicitly specifies this machine's public relay endpoint array (`ip:port`). When set, skips reflector probing and uses it directly as E and binds the relay socket to that port; for the scenario where the VPS sits behind a **egress-random (symmetric) NAT gateway** and must switch to a fixed DNAT inbound. One port per concurrent pair, see "Blind forwarding relay via VPS" |
