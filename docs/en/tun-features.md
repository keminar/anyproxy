...
Startup log `TUN bypass: device="enp3s0" ip="192.168.144.183" ...`: escape is
...
- All outbound sources are physical NIC IPs (e.g. `192.168.144.183%enp3s0`), no `10.9.0.1`, single-digit connection counts per target → **normal**, just noisy telemetry.
