...
All addresses below are anonymized RFC1918 examples and do not identify the original network.
...
Server:  UnKnown
Address:  10.20.0.53       # resolver points to the OpenVPN tunnel's intranet DNS
...
- **OpenVPN Connect TAP**: `TAP-Windows Adapter V9`, IP `10.20.30.70/30` (mask `255.255.255.252`, the local direct segment contains only `10.20.30.68 ~ 10.20.30.71`), DNS server `10.20.0.53`, **no default gateway**.

Key point: **DNS `10.20.0.53` is NOT in the TAP's `/30` direct segment** — it is at the tunnel peer and can only be reached via a `10.20.0.53/32 → TAP` route that OpenVPN installs.

## Root Cause

Windows routing uses **longest-prefix-match first**, so `/32` always beats anyproxy's `/1`:

- **/32 present**: queries to `10.20.0.53` go straight into the TAP → go through the VPN's DNS → normal, anyproxy never participates.
- **/32 deleted**: the only route to `10.20.0.53` left is anyproxy's `0/1` → the packet is sucked into anyproxy's TUN → anyproxy has no way to send it back to the TAP:
  - force-bind the physical NIC (`IP_UNICAST_IF`) → there is no `10.20.0.53` on the physical network → **blackhole**;
...
`isLocalNet` / `dstInBypassNet` (`proto/dialer_*.go`, `tun/udp.go`) **only judge "local direct subnet"** (`config.TUNBypassNets`, collected by `initBypassNets` from each NIC's direct segment), **with no RFC1918 check**. `10.20.0.53` does not belong to any local direct subnet (the TAP only gave `/30`), so `isLocalNet(10.20.0.53) == false`.

Even changing it to return true for private ranges is useless — it only swaps "blackhole" for "routing loop": after deleting the /32, the only route to `10.20.0.53` in the system is anyproxy's `0/1`, and a normal dial still loops back to itself. **The source of truth is the routing table**; code-level judgments cannot conjure a route to someone else's tunnel.
...
1. **Don't delete that /32 (easiest)**: reconnect OpenVPN Connect to let it push again, or add it manually:
   ...
   :: Pin 10.20.0.53 back to the TAP (net30 topology peer is usually .69; using the interface index is safest)
   route add 10.20.0.53 mask 255.255.255.255 10.20.30.69 if <TAP Idx> metric 1
   ```
...
2. **Catch the VPN segment at the code level (to be implemented, option B)**: add a direct-exception config for "binding to a specified interface ifindex by destination subnet" (`tun.bypassRoutes`, unrelated to mode=bypass). When TCP `tunDial` and UDP `listenUDP` match, they point `IP_UNICAST_IF` to **the TAP's index** (instead of the physical NIC), so anyproxy can actively send VPN intranet traffic (including DNS) back to the TAP even without the system /32:
   ...
   tun:
     bypassRoutes:
       - net: 10.20.0.0/24     # or precisely 10.20.0.53/32
...
- If `10.20.0.53/32` (next hop pointing to the TAP, e.g. `10.20.30.69`) is **not in the list**, the route to the VPN DNS is lost and DNS gets swallowed by anyproxy's `0/1`.
...
```cmd
:: Use the current default resolver (may be 10.20.0.53)
nslookup baidu.com
...
- `10.20.0.53` times out, `192.168.1.1` works → the /32 route to the VPN DNS is lost (this incident).
