...
Topology: residential broadband A (China Unicom `36.251.232.138`), residential broadband C (China Telecom `125.77.254.226`), public cloud host VPS (Tencent Cloud `49.232.243.123`, 1:1 NAT + stateful security group).
...
This is **deterministic**, not relying on guessing B's latency. `directPunchFirstDelay` (a constant in `nat/direct.go`, 2s,
