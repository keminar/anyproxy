...
- [direct-handshake-flow.md](direct-handshake-flow.md) — The full direct/relay handshake and where its time goes: who sends first, the ordering invariant, when the VPS actually starts forwarding (only once both legs have been heard from), how the token and the cert fingerprint are minted and flow, the three punch time quantities (budget / send window / settle window), a phase-by-phase breakdown of one real 5.3s connection, and the two landed optimizations (path-selection settle window, two-phase offer); includes five diagrams and a log cheat sheet.
...
- [windows-wol.md](windows-wol.md) — Configuring Wake-on-LAN (WOL) on Windows: NIC power-management and magic-packet wake switches, disabling Fast Startup and PCIe power saving, enabling Wake On LAN and — mandatory — disabling ErP in BIOS, plus the hard requirements (wired NIC, mains power kept on, same LAN); to send the magic packet see the `-wol` flag in [cli.md](cli.md).
