...
| `websocket.client.direct.relayPublic` | Optional, explicitly specify this machine's public relay endpoint array (`ip:port`); when configured skips reflector discovery, for VPS behind egress-randomized (symmetric) NAT gateway | — |
