...
拓扑：居民宽带 A（联通 `36.251.232.138`）、居民宽带 C（电信 `125.77.254.226`）、
公网云主机 VPS（腾讯云 `49.232.243.123`，1:1 NAT + 有状态安全组）。
...
这是**确定性**的，不靠猜 B 的延迟。`directPunchFirstDelay`（`nat/direct.go` 里的常量，2s，
