...
| 参数 | 说明 | 配置文件等价项 | 优先级 |
|------|------|----------------|--------|
| `-l ADDRPORT` | 监听地址端口，如 `:3000` 或 `127.0.0.1:3000`；可用逗号分隔写多个地址，同时绑定多个内网网段的具体 IP，如 `192.168.1.5:3000,10.0.0.5:3000`（不必监听 `0.0.0.0`，也不受限于只能绑一个 IP）；设为 `off`（或 `none`/`-`）**关闭代理监听**，仅跑 websocket/tun 等后台服务（纯内网穿透用） | `listen` | 命令行 > 配置 > `:3000` |
