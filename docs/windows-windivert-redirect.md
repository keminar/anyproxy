...
### 绑全接口的安全处理：只服务本机重定向连接

绑 `:port` 意味着该端口在所有网卡上都可达，看似能被外部主机直连。为避免它成为一个对外暴露的开放端口，`handle()` 在 Accept 时做了一道**来源校验**：

- 被重定向的连接由 `rewriteForward` 把**目的 IP 改成「包自身的源 IP」**（本机 NIC IP），源 IP 不变——所以合法的重定向连接**远端 IP 恒等于本地 IP**（都是本机某 NIC IP）。
- 外部主机直连 `NIC_IP:ProxyPort` 时，其远端 IP 是攻击者的 IP，**不等于**本地 IP → 直接拒绝。
- 回环（`127.0.0.1`/`::1`）来源也放行（本机进程，安全）。

即：`远端IP == 本地IP（或回环）` 才服务，否则丢弃。这在不改动 NAT/环回寻址逻辑的前提下，把该端口限制为**仅本机可用**。即便来源 IP 被伪造绕过这道校验，后面的 NAT 表按源端口（natPort）反查仍会失败（外部连接的源端口不在 `[10000,40000]` 且无对应条目），构成第二道防线。
...
### 私网 / VPN 内网地址（如 `192.168.1.18`）到底走哪条

Windows 上 `tun.windows.bypassPrivate` **不配默认 `true`**，所以私网地址默认**直连、不进引擎**：

| 目标（默认 `bypassPrivate=true`） | 判定 | 结果 |
|------|------|------|
| `192.168.1.18:443` / `:80` / 任意端口 | `isPrivateOrLinkLocal` → `isDirect=true` | **不转**，捕获层直接放行，直连内网 |
| 把 `192.168.1.0/24` 配进 `tun.bypassIPs` | `isExcludedIP` 命中（同样直连） | **不转**，直接放行 |

若显式配 `tun.windows.bypassPrivate: false`（想让私网也按 router 规则走）：

| 目标（`bypassPrivate=false`） | 判定 | 结果 |
|------|------|------|
| `192.168.1.18:443` / `:80` | `isDirect=false`；443/80 在 `RedirectPorts` → `shouldRedirect=true` | **走 NAT 转发（Leg 1）**：转本地代理，router 规则判定（通常直连，Leg 2 直接拨该 IP、egress 段放行不成环） |
| `192.168.1.18:22`（SSH）、`:445`（SMB）等非 80/443 | 不在 `RedirectPorts` → `shouldRedirect=false` | **不转**，包层直接放行 |
