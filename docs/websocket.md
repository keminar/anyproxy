...
- **能者多劳：谁先干完自己手头那片，就去抢下一片，不会空等**。以前是把块数按连接数死分成固定的 1:1 份额，一条连接传得快、自己那份传完了也只能干等着，慢的那条还在后面墨迹，总耗时被最慢的那条拖累；现在是所有连接从同一份"待传队列"里抢活，块数故意切得比连接数多几倍，就是为了让快的连接能连续多抢几片、把慢连接空出来的产能吃掉。进度行里 `conn1: ... conn2: ...` 现在是"这条连接迄今为止总共传了多少"，累计的是它抢到的全部片数，不是某一片单独的进度；每条连接后面还带一个 `piece=`，是它**当前**正在传的这一片有多大——第一片固定是探测片（`probeChunkSize`），测完速度后每条连接各自按 `chunkSizeForRate` 动态决定后续片的大小，`piece=` 就是用来看某条连接是不是因为测速偏低被分到了明显更小的片、一直追不上别的连接。
