...
| 角色 | 怎么判定 | 职责 |
|------|----------|------|
| **服务端**（公网侧） | 配了 `websocket.listen`（或 `-ws-listen`） | 起 websocket 监听 `/ws`，等订阅端接入；对外接收流量并经 ws 转发 |
| **订阅端**（内网侧） | 配了 `websocket.connect`（或 `-ws-connect`） | 主动回连服务端、鉴权、订阅；收到服务端来的连接后 dial 内网目标 |
...
- 服务端：`websocket.forward[].listen` 每条起一个裸 TCP 监听（`nat/forward.go` 的 `StartForward`/`listenForward`），每个进来的连接按该规则的 `email` 找订阅端（`GetClientByEmail`）桥接。
- 订阅端：`websocket.forward[].port → target` 建映射（`SetLocalForward`）。收到服务端「入口端口 Port」来的连接时，dial 对应 `target`；**Port 未在本地映射则拒绝**（`nat/forward.go` 的 `dialForCreate`）——天然白名单。
- 适合：SSH 打洞、暴露内网 Web/DB 等任意 TCP 服务。

## 配置字段

`websocket` 段（[router.go](../utils/conf/router.go) 的 `Websocket`）：

| 字段 | 角色 | 说明 |
|------|------|------|
| `listen` | 服务端 | websocket 监听地址，如 `:3002`（订阅端连它的 `/ws`）。等价 `-ws-listen` |
| `connect` | 订阅端 | 要回连的服务端 ws 地址，如 `<公网IP>:3002`。等价 `-ws-connect` |
| `host` | 订阅端 | `connect` 用的 `Host` 头/域名（走 TLS 网关时需要；无则可填服务端 IP） |
| `user` | 两端 | 鉴权用户，**两端必须一致** |
| `pass` | 两端 | 鉴权密码，**两端必须一致**；参与 token 计算，订阅端漏配会鉴权失败 |
| `email` | 订阅端 | 本订阅端身份，用于服务端定位/选择（HTTP 路径辅助、TCP 路径按它匹配 `forward.email`）。非空且不参与 token |
| `subscribe` | 订阅端 | HTTP 头订阅规则数组，每条 `{key, val}`；路径 A 用 |
| `forward` | 两端 | 裸 TCP 转发规则数组（路径 B）；同一结构两端字段含义不同，见下 |
...
- **`email` 对不上** → 裸 TCP 转发时服务端日志 `no forward ... no subscriber for email ...`。服务端 `forward.email` 必须等于某订阅端的 `websocket.email`。
