...
- **同名文件与断点续传可以先比内容再让人选（`-conflict`）**。有两种互相独立的情况：
  - **目标文件名上已有完整文件**：对比 SHA-256，不管**内容一致**还是**内容不同**，提供的选项都一样：改名重传 / 覆盖 / 跳过——要不要覆盖是使用者的明确选择，跟内容是否一致无关（`receive.allow` 里的发送方本来就有写这个目录的权限，不再单设开关；非交互的 `-conflict overwrite` 本来就不比较内容直接覆盖）。「改名重传」指**新传的文件**换个名字保存，已有文件原封不动——提示文案里写的是 `[r]ename incoming and transfer`，点名是新传的这份，免得误会成改的是已有那份。**目标文件名不接受续传**：传输总是先写 `.part`、收全校验后才改成目标名，目标名上的一定是完整文件。
  - **上次中断留下了 `.part` 临时文件**（`x.zip.<16位十六进制>.part`）：若它恰好是新文件的开头一段（逐字节哈希核对过），可以从断点续传，收完再改成目标名（目标名已被占用就照常改名，不覆盖）。**传输中断时已收到的部分会保留在 `.part` 里**（一个字节都没收到才删）；分块并行（`-parallel`）的 `.part` 带 `.chunks` 标记、中间可能有空洞，不参与续传。`.part` 不会自动清理，不再需要时手工删除。
  ...
  - 覆盖先把新内容收进 `.part`、校验通过后才替换已有文件，中途出错已有文件原封不动。续传接管 `.part` 前会校验文件名（必须是该目标名自己的 `.part`）、当前没有别的传输在写它、大小与协商时一致；尾部摘要不对会截回原长度。续传只发一段尾巴，不做 `-parallel` 分块。
...
- **能者多劳：谁先干完自己手头那片，就去抢下一片，不会空等**。以前是把块数按连接数死分成固定的 1:1 份额，一条连接传得快、自己那份传完了也只能干等着，慢的那条还在后面墨迹，总耗时被最慢的那条拖累；现在是所有连接从同一份"待传队列"里抢活，块数故意切得比连接数多几倍，就是为了让快的连接能连续多抢几片、把慢连接空出来的产能吃掉。进度行里 `conn1: ... conn2: ...` 现在是"这条连接迄今为止总共传了多少"，累计的是它抢到的全部片数，不是某一片单独的进度。
