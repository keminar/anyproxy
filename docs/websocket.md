...
- **不覆盖同名文件**，自动改成带序号的名字，**按接收方系统习惯命名**：Windows/其它 `x (1).zip`，Linux `x.zip.1`，macOS `x copy.zip` / `x copy 2.zip`。覆盖会悄无声息毁掉收方已有的数据，代价远大于多一个带序号的名字；实际存成什么名字会回报给发送端。改名字这一步是"用 `O_CREATE|O_EXCL` 原子认领名字"而不是"先探测存不存在、再单独一步 Rename"——两步分开在两个独立进程之间不是原子的：两个终端并发发同名文件时，双方都可能在探测那一刻看到"不存在"，都选中同一个名字去 Rename。`os.Rename` 对已存在的目标是直接覆盖（Go 在 Windows 上用 `MOVEFILE_REPLACE_EXISTING` 特意抹平了跟 POSIX `rename()` 的差异，两边行为一致），后一个会悄悄吃掉前一个刚落盘、且已经回复过"Saved"的文件——这不是 Windows 特有的问题，Linux/macOS 上同样会撞上。
- **同名文件可以先比内容再让人选（`-conflict`）**。传之前对比 SHA-256：**内容一致**→ 改名重传 / 覆盖 / 跳过；**内容不同**→ 续传（仅当已有文件恰好是新文件的开头一段，逐字节哈希核对过）/ 改名重传 / 跳过。选项直接回车 = 跳过，大写字母（`R`/`O`/`C`/`S`）= 对之后所有冲突都这样处理。`-conflict` 取值：`ask`（终端里的默认，逐个问）、`rename`（自动改名，标准输入不是终端时的默认，也就是旧行为）、`overwrite`、`skip`、`resume`（能续就续、已完整就跳过、不能续就改名）。`-send` 与 `-recv` 都支持：`-send` 时冲突在**对端**（多一次 probe 往返，收方回 stat 结果，已有文件不比来件大时才算哈希），`-recv` 时冲突在**本机**（用 pull 的 `hash` 操作向对端要前 N 字节的哈希）。
  - **「改名」指新传的文件换个名字保存，已有文件原封不动**；覆盖/续传才会改动已有文件，且只在使用者明确选择时发生（`receive.allow` 里的发送方本来就有写这个目录的权限，不再单设开关）。
  - 覆盖先把新内容收进 `.part`、校验通过后才替换已有文件，中途出错已有文件原封不动。续传直接接在已有文件末尾写，尾部摘要不对或中途断线会把文件**截回原长度**。续传只发一段尾巴，不做 `-parallel` 分块。
