...
- **同名文件与断点续传可以先比内容再让人选（`-conflict`）**。有两种互相独立的情况：
  - **目标文件名上已有完整文件**：对比 SHA-256，**内容一致**→ 改名重传 / 覆盖 / 跳过；**内容不同**→ 改名重传 / 跳过。「改名」指新传的文件换个名字保存，已有文件原封不动；覆盖只在使用者明确选择时发生（`receive.allow` 里的发送方本来就有写这个目录的权限，不再单设开关）。**目标文件名不接受续传**：传输总是先写 `.part`、收全校验后才改成目标名，目标名上的一定是完整文件。
...
默认每个文件只占一条连接（`direct`/VPS 盲转发下是一条独立打洞的 QUIC 连接，`relay` 下是一次中继会话），这在单条连接吞吐已经打满链路时够用，但受限于单流拥塞窗口爬升、中继流控窗口这些"单条连接"自身的天花板时就不够了。`-parallel N`（默认 1，硬上限 4——超过的会被截到 4）让**单个**大文件按字节区间切成最多 `N` 块并行传，两条路径（`-via direct`/VPS、`-via relay`）和两个方向（`-send`/`-recv`）都支持：
...
- **文件太小不会被切**：低于 8MiB 的文件永远走单连接路径，`-parallel` 的值被忽略，切块的握手/首部开销在小文件上不划算；块数不随文件大小无限增长，最多也就是 4 块（16MiB 以上的文件），再大的文件一样切 4 块，不会因为是几个 GB 就多开洞。
- **失败语义与不切块时一致，但容忍部分连接打不通**：`-via direct`/VPS 下, `N` 条独立连接只要至少打通 1 条就会用打通的那几条继续传（少数候选路径不通是正常现象, 不该因此让整个传输失败）；连接建好之后, 任意一块传输失败（网络错误、校验不过）仍然让整份文件报错、`.part` 文件被清理, 不会留下半成品, 也不会自动重试。
- **`-via relay` 下每一块各自协商一次加密会话**（各自独立的随机 salt、独立派生的 AES-256-GCM 密钥），协议本身早就支持"随时开一条新的加密会话"，不需要为切块单独改握手。
- **传输过程中的进度行会带上每条连接各自的即时速率**（`conn1: ... conn2: ...`），方便看出是不是有条连接明显拖后腿。
