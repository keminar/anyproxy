...
**本地端口**是我们自己绑的，进程不重启就不变；但**对端能用的那个端点（外网 IP + 外网端口）完全不受我们控制**：

- 地址会变——IPv6 隐私临时地址通常几小时到一天轮换一次，ISP 前缀也可能变；
- 端口也会变——路径上若有 NAT / 端口映射（IPv6 少见但存在，如 NAT66/NPTv6、CPE 改写），外网端口和本地端口就不是一回事；映射老化重建后还会换一个。

所以协议里**从不传本地端口**，传的一律是反射器探测到的完整端点。具体做法：

- **A 侧**：每次发起请求前重新探测，不缓存。
- **C 侧**：每 25 秒重探一次，变了就重新通告（`keepAnnounced`）。这个间隔要小于常见的 NAT/防火墙 UDP 映射老化时间（常见 30 秒起），探测包本身就兼做保活——间隔一旦超过老化时间，映射会先失效再重建，外网端口可能随之改变，而服务端手里还是旧端点，对端就会拨到死地址。

### 空闲连接会自动关闭

QUIC 连接开着 20 秒 keep-alive（活跃会话期间要靠它焐住 IPv6 有状态防火墙的洞，RDP 常有大段没数据的时候）。但 keep-alive 会一直把 QUIC 自己的空闲超时顶回去，连接不会自然死亡，所以有一个独立的回收：**一条连接上所有会话都结束、且空闲超过 90 秒后主动关闭**（`reapSessions`）。

不留着长期复用是有意的：留着省下的只是下次那一两秒建连，代价却是保活包无限期发下去，而且多台 A 连过同一个 C 时，C 会永久累积连接。跨过地址轮换的连接本来也已经是死的——"复用"在最需要它的时候恰恰不可靠。
