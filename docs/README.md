...
- [proxy-decision.md](proxy-decision.md) — 代理决策逻辑详解：`target`(local/remote/auto/deny) × `proxy`(多代理 + local/deny 后缀) 的完整判定顺序、host/全局回退链、热加载、代理每次实探(不缓存不可用)。
...
- [windows-windivert-redirect.md](windows-windivert-redirect.md) — Windows WinDivert 重定向原理：数据包捕获→NAT 改写→本地代理→恢复目的地转发的完整链路；NAT 表、dual-stack 监听、两条连接与直连判定、`bypassPrivate`(私网/VM 默认直连)、环路防护(egress 源端口段根治 IPv6 直连自环 + SOCKS guard 兜底)。
