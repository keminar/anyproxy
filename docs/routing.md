...
hosts:
  - name: golang.org
    match: contain
    ip: 203.0.113.10   # 本机解析的 192.0.2.10 不通，手动换一个可达示例 IP
```
...
> 普通单代理的全局代理走无探测快路径；一旦配成多代理或带后缀，则每个请求会按连通性挑选（含 200ms 拨号探测），与单域名 `proxy` 行为一致。
>
> **代理每次实探、不缓存不可用**：每次选代理都用 200ms 拨号实探一次，挂掉的代理下次仍会被实探。好处是**代理重启恢复后下一个请求立刻可用**，不会被旧失败缓存挡住而无响应；代价是代理长时间挂着时每个请求各花最多 200ms 探测。详见 [caching.md](caching.md) 与 [proxy-decision.md](proxy-decision.md)。
